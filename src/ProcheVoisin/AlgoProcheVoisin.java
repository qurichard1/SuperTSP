 ...
 * @author qurichard1
 */
public class AlgoProcheVoisin {
    
    
    private static HashMap<String,List<Ville>> map;
    
        
    
    public static Monde projAlgoProcheVoisin(Monde m) {
        map=new HashMap();
        map=remplirMap(m,map);
    ...
    public static HashMap<String,List<Ville>> remplirMap(Monde m,HashMap<String,List<Ville>> map){
        String vKey;
        List<Ville> temp;
        for(Ville v:m.tabVille){
            vKey=String.format("%d/%d",(int) v.getLongitude(),(int)v.getLatitude());
